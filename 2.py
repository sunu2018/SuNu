...
line = LINE('EutgXtt7c6fsjBEvt0w1.qYrMQO4gXpceGGR1gbkR4q.e/wo1L15MZUSCNYOUc4gCiIO0pq6zuhi9CJvxdZuTZA=')
...
settings = {
    ...
    "KIEBOT1":"\n[ยังไม่มีข้อความเข้า]",
    "KIEBOT2":"\n[ยังไม่มีข้อความออก]",
    "tag2":"\n[ราคาเซลฟ์บอทตามจำนวนคิกเกอรสั่งทำเฉพาะเซลบอทกันรัน 250 บาทเชลฟ์บอท+kicker  แค่เชลฟ์บอท300 เพิ่มkickerได้4ตัว 1 คิก 50 บาท 2 คิก 100 บาท 3 คิก 150 บาท 4 คิก 200 บาท]",
    "tag1":"""\n[─┅═✥KIEBOTLINE✥═┅─""",
    "blacklist":{},
    "winvite":False,
    "wblacklist":{},
...
#==============================================================================#
                    ...
                    line.sendMessage(to, "🌺ออโต้บล็อคเปิดแล้ว🌺")
                elif text.lower() == 'block off':
                    settings["autoAdd"] = False
                    line.sendMessage(to, "🌺ออโต้บล็อคปิดแล้ว🌺")
                elif text.lower() == 'join on':
                    settings["autoJoin"] = True
                    line.sendMessage(to, "🌺ออโต้เข้ากลุ่มเปิดแล้ว🌺")
                elif text.lower() == 'join off':
                    settings["autoJoin"] = False
                    line.sendMessage(to, "🌺ออโต้เข้ากลุ่มปิดแล้ว🌺")
...
#--------------------------------------------------------
                    ...
                    line.sendMessage(to, "🌺ออโต้ออกแชทรวมเปิดแล้ว🌺")
                elif text.lower() == 'leave off':
                    settings["autoLeave"] = False
                    line.sendMessage(to, "🌺ออโต้ออกแชทรวมปิดแล้ว🌺")
                elif text.lower() == 'read on':
                    settings["autoRead"] = True
                    line.sendMessage(to, "🌺อ่านออโต้เปิดแล้ว🌺")
                elif text.lower() == 'read off':
                    settings["autoRead"] = False
                    line.sendMessage(to, "🌺อ่านออโต้ปิดแล้ว🌺")
                elif text.lower() == 'sticker on':
                    settings["checkSticker"] = True
                    line.sendMessage(to, "🌺อ่านสติ้กเกอร์เปิดแล้ว🌺")
                elif text.lower() == 'sticker off':
                    settings["checkSticker"] = False
                    line.sendMessage(to, "🌺อ่านสติ้กเกอร์ปิดแล้ว🌺")      
                    ...
                    if line != None:
                        if 'MENTION' in list(msg.contentMetadata.keys())!= None:
                            ...
                            for ls in lists:
                                ...
                                line.sendImageWithURL(msg.to, str(path))
                elif msg.text.lower().startswith("ก้อปปี้ "):
                    if 'MENTION' in list(msg.contentMetadata.keys())!= None:
                        names = re.findall(r'@(\w+)', text)
                        mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                        mentionees = mention['MENTIONEES']
...
#==============================================================================#
                elif msg.text.lower().startswith("เลียนแบบเปิด "):
                    targets = []
                    ...
                    for target in targets:
                        try:
                            ...
                            break
                elif msg.text.lower().startswith("เลียนแบบปิด "):
                    targets = []
                    key = eval(msg.contentMetadata["MENTION"])
                    key["MENTIONEES"][0]["M"]
                    for x in key["MENTIONEES"]:
                    ...
                    for target in targets:
                        try:
                            ...
                            break
                elif text.lower() == 'บันชีเลียนแบบ':
                    if settings["mimic"]["target"] == {}:
                        line.sendMessage(msg.to,"Tidak Ada Target")
                    else:
                        mc = "╔══[ 🍒รายชื่อผู้โดนเลียนแบบ🍒 ]"
...
#==============================================================================#
                ...
                elif msg.text.lower().startswith("มอง"):
                    data = msg.text[len("มอง"):].strip()
                    if data == "":
                    ...
                    if GACReq.responseNum == 0:
                        ...
                        if msg.toType != 0:
                                ...
                                line.sendMessager(msg.from_,"พบข้อผิดพลาดที่ไม่รู้จัก (｀・ω・´)")
         ...
         #             settings["KIEBOT2"] = msg.text.replace("Tx2: ","")
          #            line.sendMessage(msg.to,"❥คุณได้ตั้งข้อความออกแล้ว")
                    ...
                    if c in [""," ","\n",None]:
                       ...
                       settings["KIEBOT1"] = c
                       line.sendMessage(msg.to,"➠ ตั้งค่าข้อความต้อนรับ👌\n\n" + c)
                    ...
                    if c in [""," ","\n",None]:
                        ...
                        settings["KIEBOT2"] = c
                        line.sendMessage(msg.to,"➠ ตั้งค่าข้อความกล่าวถึงคนออกจากกลุ่ม👌\n\n" + c)
...
#===========≠==========================================#
                ...
                elif "ดำทั้งหมด" in msg.text:
                  if msg._from in Family:
                      if msg.toType == 2:
                           print ("All Banlist")
                           _name = msg.text.replace("ดำทั้งหมด","")
                           gs = line.getGroup(msg.to)
...
# ----------------- NOTIFED MEMBER JOIN GROUP
        if op.type == 17:
         ...
         #   kie = line.acceptGroupInvitation(op.param1)
            ...
            line.sendMessage(op.param1,"@! " + line.getContact(op.param2).displayName + "ยินดีต้อนรับ ☞ " + str(settings["KIEBOT1"]))
